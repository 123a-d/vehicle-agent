...
每个工具由 `agent_tools.py` 中的 `ToolDefinition` 统一描述名称、Schema 和 Python Handler，并自动生成 DeepSeek `TOOLS` 与 Python `TOOL_REGISTRY`。公共 Agent Loop 位于 `agent_core.py`，负责 Tool Call 解析、执行、结果回填和最大步数保护；`multi_tool_agent.py` 与 `session_agent.py` 分别负责单次任务和多轮 Session 上下文。
...
数据库、模型、上传目录、最大文件大小和日志级别也可通过 `.env.example` 中列出的环境变量覆盖；默认值集中定义在 `config.py`。程序不会自动加载 `.env`，也不会在日志中记录 API Key、Authorization、完整 Prompt 或图片内容。
...
测试覆盖 Tool Definition/Registry、Dispatcher、Agent Loop、多工具调用、Session 隔离、临时 SQLite 和 FastAPI 上传及错误路径。测试使用 Mock LLM 和临时数据库，不调用真实 DeepSeek 或 ResNet18。
...
|-- agent_core.py                   # 公共 Agent Loop
|-- agent_tools.py                  # ToolDefinition、Schema、Registry 与调度器
|-- config.py                       # 环境与路径配置
|-- logging_config.py               # 统一日志入口
|-- multi_tool_agent.py             # 单次多工具 Agent 入口
